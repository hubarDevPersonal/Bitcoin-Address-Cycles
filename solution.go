...
func (s *SolutionServer) findCycleDFS(startVertex *rpc.Vertex, currentVertex *rpc.Vertex, maxCycleLength int64, visited map[int64]bool, currentCycleLength int64) int64 {
				...
				if input.Index == currentVertex.Index {
					...
					for _, output := range transaction.Outputs {
						if output.Address == startVertex.Address {
							...
							fmt.Println(output.Address, startVertex.Address, output.Index, startVertex.Index)
							cycleCount++
						}
